...
I<void>E<10> E<8>E<8>
B<cronkite_seturl>(I<const char *urlfmt>);
...
B<cronkite_seturl>() is used to set the aur target url. Expected strings are
either an http url, or a file url. For example:

    // http url
    cronkite_seturl("http://aur.archlinux.org/rpc.php?type=%s&arg=%s");
    // local file url
    cronkite_seturl("file:///tmp/test.json");
